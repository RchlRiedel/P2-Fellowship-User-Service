...
function getAllUsers() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            //send query
            let results = yield client.query(`select * from ${schema}.users u;`);
            //return results
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (updatedUser.username) {
                yield client.query(`update ${schema}.users set "username" = $1 where user_id = $2;`, [updatedUser.username, updatedUser.userId]);
            }
            if (updatedUser.password) {
                yield client.query(`update ${schema}.users set "password" = $1 where user_id = $2;`, [updatedUser.password, updatedUser.userId]);
            }
            if (updatedUser.firstName) {
                yield client.query(`update ${schema}.users set "first_name" = $1 where user_id = $2;`, [updatedUser.firstName, updatedUser.userId]);
            }
            if (updatedUser.lastName) {
                yield client.query(`update ${schema}.users set "last_name" = $1 where user_id = $2;`, [updatedUser.lastName, updatedUser.userId]);
            }
            if (updatedUser.email) {
                yield client.query(`update ${schema}.users set "email" = $1 where user_id = $2;`, [updatedUser.email, updatedUser.userId]);
            }
            if (updatedUser.affiliation) {
                yield client.query(`update ${schema}.users set "affiliation" = $1 where user_id = $2;`, [updatedUser.affiliation, updatedUser.userId]);
            }
            if (updatedUser.placesVisited) {
                yield client.query(`update ${schema}.users set "places_visited" = $1 where user_id = $2;`, [updatedUser.placesVisited, updatedUser.userId]);
            }
            if (updatedUser.address) {
                yield client.query(`update ${schema}.users set "address" = $1 where user_id = $2;`, [updatedUser.address, updatedUser.userId]);
            }
            if (updatedUser.role) { //figure out what you're doing for this...
                yield client.query(`update ${schema}.users set "role" = $1 where user_id = $2;`, [updatedUser.role, updatedUser.userId]);
            }
            if (updatedUser.image) { //figure out what you're doing for this...
                yield client.query(`update ${schema}.users set "image" = $1 where user_id = $2;`, [updatedUser.image, updatedUser.userId]);
            }
...
function saveNewUser(newUser) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            client = yield _1.connectionPool.connect();
            let results = yield client.query(`insert into ${schema}.users ("username", "password", "first_name", "last_name", "email", "affiliation", "places_visited", "address", "role", "image")
