...
export async function saveNewUserService(newUser: User): Promise<User> {
    ...
    try {
        ...
        if (newUser.image) { //avoid splitting a void string!
            ...
            savedUser = await saveNewUser(newUser)
...
export async function updateUserService(updatedUser: User): Promise<User>{
    try {
        ...
        if (updatedUser.image) {
            ...
            updatedUser.image = `${bucketBaseUrl}/LOTR_Profiles/${userInfo.username}/${date}.${contentType}`
            savedUser = await updateUser(updatedUser)
            await saveProfilePicture(contentType, imageBase64Data, `LOTR_Profiles/${userInfo.username}/${date}.${contentType}`)
